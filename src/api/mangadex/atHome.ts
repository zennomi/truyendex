 ...
 */
export const getAtHomeServerChapterId = function (
  chapterId: string,
  options?: GetAtHomeServerChapterIdRequestOptions,
) {
  if (chapterId === undefined) {
    return Promise.reject(
      "ERROR - getAtHomeServerChapterId: Parameter `chapterId` cannot be undefined",
    );
  } else if (chapterId === "") {
    return Promise.reject(
      "ERROR - getAtHomeServerChapterId: Parameter `chapterId` cannot be blank",
    );
  }
  ...
  return util.createHttpsRequestPromise<GetAtHomeServerChapterIdResponse>(
    "GET",
    path,
  );
};
