 ...
 */
export const postCaptchaSolve = function (
  challengeString: string,
  token?: AuthenticationToken,
) {
  if (challengeString === undefined) {
    return Promise.reject(
      "ERROR - postCaptchaSolve: Parameter `challengeString` cannot be undefined",
    );
  } else if (challengeString === "") {
    return Promise.reject(
      "ERROR - postCaptchaSolve: Parameter `challengeString` cannot be blank",
    );
  }
  ...
  try {
    ...
    return util.createHttpsRequestPromise<PostCaptchaSolveResponse>(
      "POST",
      path,
      httpsRequestOptions,
    );
