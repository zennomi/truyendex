  ...
  return (
    <div className="list-chapter mb-2" id="nt_listchapter">
          ...
          {chapters.map((chapter) => (
            <li className="row" key={chapter.id}>
              <div className="col-xs-5 chapter" key={chapter.id}>
                <Link href={Constants.Routes.nettrom.chapter(chapter.id)}>
                  {Utils.Mangadex.getChapterTitle(chapter)}
                </Link>
              </div>
              <div className="col-xs-4 no-wrap small text-center">
      ...
      <ReactPaginate
        ...
        pageCount={
          Math.floor(
            chapterListData.total / Constants.Mangadex.CHAPTER_LIST_LIMIT,
          ) + 1
        }
        previousLabel="<"
