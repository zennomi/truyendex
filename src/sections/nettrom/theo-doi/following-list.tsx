...
export default function FollowingList() {
    useAuth({ middleware: 'auth', redirectIfNotAuthenticated: routes.loginWithRedirect(routes.nettrom.following) })
    const { updateMangas, updateMangaStatistics, mangaStatistics, mangas } = useMangadex()
                                                ...
                                                <div className="image">
                                                    ...
                                                    <div className="view clearfix">
                                                        <span className="pull-left">
                                                            <i className="fa fa-star"></i> {mangaStatistics[series_id] && Math.round((mangaStatistics[series_id].rating.bayesian || 0) * 10) / 10 || "N/A"}{" "}
                                                            <i className="fa fa-comment" /> {mangaStatistics[series_id] && mangaStatistics[series_id].comments?.repliesCount || "N/A"}{" "}
                                                            <i className="fa fa-heart" /> {mangaStatistics[series_id] && mangaStatistics[series_id].follows || "N/A"}
                                                        </span>
                                                    </div>
                                                </div>
                                                <figcaption>
                                                    <div className="follow-action clearfix">
                                                        ...
                                                        >
                                                            {" "}
                                                            <i className="fa fa-times" /> <span>Bỏ theo dõi</span>{" "}
                                                        </a>{" "}
                                                    </div>
                                                    <h3>
                                                        {" "}
